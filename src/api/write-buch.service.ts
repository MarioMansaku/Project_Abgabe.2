...
type PostResponse =
    operations['BuchWriteController_post']['responses']['201']['content'];
type PutPayload =
    operations['BuchWriteController_put']['requestBody']['content']['application/json'];
type PutResponse =
    operations['BuchWriteController_put']['responses']['204']['content'];
type DeleteResponse204 =
    operations['BuchWriteController_delete']['responses']['204']['headers'];
...
export class WriteServiceBuch {
    ...
    async postBuch(buch: PostPayload): Promise<PostResponse | void> {
        ...
        if (response.status !== 201) {
            throw new Error(
                `Das Buch wurde nicht erfolgreich angelegt. Statuscode: ${response.status}`,
            );
        }
    ...
    async putBuch(buch: PutPayload, id: number): Promise<PutResponse | void> {
        ...
        const response: AxiosResponse<PutResponse> = await axios.put(
            url,
            buch,
            {
                headers: {
        ...
        if (response.status !== 204) {
            throw new Error(
                `Das Buch wurde nicht erflgreich aktualisiert. Statuscode: ${response.status}`,
            );
        }
    ...
    async deleteBuch(id: number): Promise<DeleteResponse204 | void> {
        const url = `${BASE_URL}/${id}`;
        const response: AxiosResponse<DeleteResponse204> = await axios.delete(
            url,
            {
                headers: {
                    Authorization: `Bearer ${token}`,
        ...
        if (response.status !== 204) {
            throw new Error(
                `Das Buch wurde nicht erfolgreich gelöscht. Statuscode:${response.status}`,
            );
        }
